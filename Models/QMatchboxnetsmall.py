...
class QMatchBoxNet(nn.Module):    
    def __init__(self, preprocessing_mode:str) -> None:
        ...
        self.QJasperBlock0 = nn.Sequential(
            QuaternionConv(in_channels=40, out_channels=32, kernel_size=11, stride=(1,), padding=5, groups =1 , bias= False),
            nn.BatchNorm1d(num_features=32,eps=0.001, momentum=0.1,affine=True,track_running_stats=True),
        ...
        self.QJasperBlock1_mconv = nn.Sequential(
            QuaternionConv(in_channels=32, out_channels=32, kernel_size=13, stride=1, padding=5, groups =1 , bias= False),
            
            nn.BatchNorm1d(num_features=32,eps=0.001, momentum=0.1,affine=True,track_running_stats=True),
        )
        
        self.QJasperBlock1_res = nn.Sequential(
            QuaternionConv(in_channels=32, out_channels=32, kernel_size=1, stride=1, bias = False),
            nn.BatchNorm1d(num_features=32, eps=0.001, momentum=0.1, affine=True, track_running_stats=True)
        ...
        self.QJasperBlock2_mconv = nn.Sequential(
            QuaternionConv(in_channels=32, out_channels=32, kernel_size=15, stride=1, padding=7, groups =1 , bias= False),
            # nn.Conv1d(in_channels=32, out_channels=32, kernel_size=(1,), stride= (1,), bias=False),
            nn.BatchNorm1d(num_features=32,eps=0.001, momentum=0.1,affine=True,track_running_stats=True),
        )
        ...
        self.QJasperBlock3_mconv = nn.Sequential(
            QuaternionConv(in_channels=32, out_channels=32, kernel_size=17, stride=1, padding=8, groups =1, bias= False),
            # nn.Conv1d(in_channels=32, out_channels=32, kernel_size=(1,), stride= (1,), bias=False),
            nn.BatchNorm1d(num_features=32,eps=0.001, momentum=0.1,affine=True,track_running_stats=True),
        )
        ...
        self.QJasperBlock4 = nn.Sequential(
            QuaternionConv(in_channels=32, out_channels=64, kernel_size=29, stride=1, padding=28, dilatation=2, groups=1, bias= False),
            # nn.Conv1d(in_channels=32, out_channels=128, kernel_size=(1,), stride= (1,), bias=False),
        ...
        self.classifier = nn.Sequential(
            nn.Linear(in_features=64, out_features=35)
            )
    ...
    def forward(self,x):
        ...
        if self.preprocessing_mode == "audio":
            ...
            o=x.squeeze() # their dataloader returns batch, channel =1,H*W, after squeeze 1 is gone then it becoms batch , channel =Height, width 
            # since you are choosing off make sure jasper block 0 takes 4 as input channel
            # print("hello")
...
# m = QMatchBoxNet(preprocessing_mode="") # since dataloader already sends mel spectrogram



# # print(m(torch.randn(32,1,40,32)).shape)

# print(summary(m, (32,1,40,32)))
    
  
# torch.randn(4,1,40,32)
# torch.randn(4,4,40,32)