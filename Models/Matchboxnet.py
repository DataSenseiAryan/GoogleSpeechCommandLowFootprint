...
class MatchBoxNet(nn.Module):
    ...
    def __init__(self, preprocessing_mode:str) -> None:
        ...
        self.preprocessing_cnn = nn.Sequential(
            nn.Conv1d(in_channels=40, out_channels=64, kernel_size=(1,), stride=(1,), padding =0, bias = False)
        )
        
        
        
        self.JasperBlock0 = nn.Sequential(
            nn.Conv1d(in_channels=64, out_channels=64, kernel_size=(11,), stride=(1,), padding=(5,), groups =64 , bias= False),
        ...
        self.JasperBlock1_mconv = nn.Sequential(
            nn.Conv1d(in_channels=128, out_channels=128, kernel_size=(13,), stride=(1,), padding=(5,), groups =128 , bias= False),
            nn.Conv1d(in_channels=128, out_channels=64, kernel_size=(1,), stride= (1,), bias=False),
        ...
        self.JasperBlock2_mconv = nn.Sequential(
            nn.Conv1d(in_channels=64, out_channels=64, kernel_size=(15,), stride=(1,), padding=(7,), groups =64 , bias= False),
        ...
        self.JasperBlock3_mconv = nn.Sequential(
            nn.Conv1d(in_channels=64, out_channels=64, kernel_size=(17,), stride=(1,), padding=(8,), groups =64 , bias= False),
            nn.Conv1d(in_channels=64, out_channels=64, kernel_size=(1,), stride= (1,), bias=False),
            nn.BatchNorm1d(num_features=64,eps=0.001, momentum=0.1,affine=True,track_running_stats=True),
        )
        ...
        self.JasperBlock4 = nn.Sequential(
            nn.Conv1d(in_channels=64, out_channels=64, kernel_size=(29,), stride=(1,), padding=(28,),dilation=(2,), groups=64, bias= False),
            nn.Conv1d(in_channels=64, out_channels=128, kernel_size=(1,), stride= (1,), bias=False),
            nn.BatchNorm1d(num_features=128, eps=0.001, momentum=0.1,affine=True,track_running_stats=True),
            nn.ReLU(inplace=True),
            nn.Dropout(p =0.0, inplace =False)
...
# m = MatchBoxNet(preprocessing_mode='cnn')
