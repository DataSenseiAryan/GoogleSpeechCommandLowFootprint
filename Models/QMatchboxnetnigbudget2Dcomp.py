...
class QMatchBoxNetBB(nn.Module):
    ...
    def __init__(self, preprocessing_mode:str) -> None:
        ...
        self.preprocessing_cnn = nn.Sequential(
            QuaternionConv(in_channels=160, out_channels=64, kernel_size=1, stride=1, padding =0, bias = False)
        )
        
        
        
        self.JasperBlock0 = nn.Sequential(
            QuaternionConv(in_channels=160, out_channels=64, kernel_size=11, stride=1, padding=5, groups =1 , bias= False),
    ...
    def forward(self,x):
        if self.preprocessing_mode == "audio":
            ...
            print("MFCC output:", o.shape)
             ...
             o=x.view(x.shape[0],4*x.shape[2],x.shape[3])
        #     o = x.squeeze()
...
# m = QMatchBoxNetBB(preprocessing_mode='off')
