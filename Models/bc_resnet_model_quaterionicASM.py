...
class NormalBlock(nn.Module):
    def __init__(self, n_chan: int, *, dilation: int = 1, dropout: float = DROPOUT, use_subspectral: bool = True):
        ...
        self.f2 = nn.Sequential(
            QuaternionConv2dASM(in_channels=n_chan, out_channels=n_chan, kernel_size=(3,1), padding="same",stride =1, groups=1),
            # nn.Conv2d(n_chan, n_chan, kernel_size=(3, 1), padding="same", groups=n_chan),
            norm_layer,
        )
        self.f1 = nn.Sequential(
            QuaternionConv2dASM(in_channels=n_chan, out_channels=n_chan, kernel_size=(1,3), padding="same",stride =1, groups=1),
            ...
            QuaternionConv2dASM(in_channels=n_chan, out_channels=n_chan, kernel_size=(1,1), padding=0,stride =1, groups=1) ,
...
class TransitionBlock(nn.Module):
    def __init__(self, in_chan: int, out_chan: int, *, dilation: int = 1, stride: int = 1, dropout: float = DROPOUT, use_subspectral: bool = True):
        ...
        if stride == 1:
            ...
            conv = QuaternionConv2dASM(in_channels=out_chan, out_channels=out_chan, kernel_size=(3,1), padding="same", groups= 1, stride=(1,1))
            
        else:
            conv = QuaternionConv2dASM(in_channels=out_chan, out_channels=out_chan, kernel_size=(3,1), stride= (stride,1) ,padding=(1,1), groups=1)            
        ...
        self.f2 = nn.Sequential(
            ...
            QuaternionConv2dASM(in_channels=in_chan, out_channels=out_chan,stride =(1,1), kernel_size=(1,1), padding=0, groups= 1),
        ...
        self.f1 = nn.Sequential(
            ...
            QuaternionConv2dASM(in_channels=out_chan, out_channels=out_chan,stride =(1,1), kernel_size=(1,3), padding="same",dilatation=(1,dilation), groups= 1),
            nn.BatchNorm2d(out_chan),
            nn.SiLU(),
            # nn.Conv2d(out_chan, out_chan, kernel_size=1),
            QuaternionConv2dASM(in_channels=out_chan, out_channels=out_chan,stride =(1,1), kernel_size=(1,1), padding=0, groups= 1),
            nn.Dropout2d(dropout)
        )
...
class QBcResNetModelASM(nn.Module):
    def __init__(self, n_class: int = 35, *, scale: int = 1, dropout: float = DROPOUT, use_subspectral: bool = True):
        super().__init__()
        self.input_conv = QuaternionConv2dASM(in_channels=4, out_channels=16*scale, stride =(2,1), kernel_size=(5,5), padding=2, groups= 1)
        ...
        self.dw_conv = QuaternionConv2dASM(in_channels=20*scale, out_channels=20*scale,stride =(1,1), kernel_size=(5,5), padding=0,dilatation=(1,1), groups= 1)
        # self.onexone_conv = nn.Conv2d(20*scale, 32*scale, kernel_size=1)
        
        self.onexone_conv = QuaternionConv2dASM(in_channels=20*scale, out_channels=32*scale,stride =(1,1), kernel_size=(1,1), padding=0,dilatation=(1,1), groups= 1)
