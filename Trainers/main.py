...
from Models.BCnobias import BcNoBias
# from QMatchboxnetsmall import QMatchBoxNet
# from QMatchboxnet80k import QMatchBoxNet80k
...
def run(model, train_loader, validation_loader, test_loader, optimizer, scheduler, device, checkpoint_file, n_epoch=10, log_interval=100):
    ...
    for epoch in range(n_epoch):
        ...
        train.train_epoch_original(model, optimizer, train_loader, device, epoch, log_interval=log_interval)
        if scheduler:
            scheduler.step()
        score = apply.compute_accuracy_original(model, validation_loader, device)
    ...
    test_score = apply.compute_accuracy_original(best_model, test_loader, device)
...
def train_command(scale, batch_size, device, epoch, log_interval, checkpoint_file, optimizer, dropout, subspectral_norm):
    ...
    # model = QMatchBoxNet(preprocessing_mode="off").to(device)
    
    # model = QMatchBoxNet80k(preprocessing_mode='off').to(device)
    # model = ResNet_182d(image_channels=1,num_classes=35).to(device)
    # model = MatchBoxNet(preprocessing_mode='cnn').to(device)
    # model = QMatchBoxNet80k(preprocessing_mode='cnn').to(device)
    # model = QMatchBoxNetBB(preprocessing_mode='cnn').to(device)
    model = BcNoBias(
        n_class=35,
        scale=8,
        dropout=0.2,
...
def test_command(model_file, scale, batch_size, device, dropout, subspectral_norm):
    ...
    # model = QMatchBoxNet80k(preprocessing_mode='off')
    # model = ResNet_182d(image_channels=1,num_classes=35).to(device)
    # model = MatchBoxNet(preprocessing_mode='cnn').to(device)
    # model = QMatchBoxNetBB(preprocessing_mode='cnn').to(device)
    ...
    test_score = apply.compute_accuracy_original(model, test_loader, device)
