...
from QResNet182Dphase import QResNet_182d
...
def run(model, train_loader, validation_loader, test_loader, optimizer, scheduler, device, checkpoint_file, n_epoch=10, log_interval=100):
    ...
    for epoch in range(n_epoch):
        ...
        train.train_epoch_phase(model, optimizer, train_loader, device, epoch, log_interval=log_interval)
        if scheduler:
            scheduler.step()   
...
def train_command(scale, batch_size, device, epoch, log_interval, checkpoint_file, optimizer, dropout, subspectral_norm):
    ...
    # model = QBcResNetModel(n_class=35, scale=6,dropout=0.2,use_subspectral=True).to(device)
    
    model = QResNet_182d(image_channels=4, num_classes=35).to(device)
    train_loader = torch.utils.data.DataLoader(
        get_data_phase.SubsetSC(subset="training"),
        batch_size=batch_size,
        shuffle=True,
        collate_fn=get_data_phase.collate_fn,
        num_workers=num_workers,
        pin_memory=pin_memory
    )
    validation_loader = torch.utils.data.DataLoader(
        get_data_phase.SubsetSC(subset="validation"),
        batch_size=batch_size,
        shuffle=False,
        drop_last=False,
        collate_fn=get_data_phase.collate_fn,
    ...
    test_loader = torch.utils.data.DataLoader(
        get_data_phase.SubsetSC(subset="testing"),
        batch_size=batch_size,
        shuffle=False,
        drop_last=False,
        collate_fn=get_data_phase.collate_fn,
...
def apply_command(model_file, wav_file, scale, device, dropout, subspectral_norm):
    ...
    model = bc_resnet_model.BcResNetModel(
        n_class=get_data_phase.N_CLASS,
        scale=scale,
        dropout=dropout,
        use_subspectral=subspectral_norm,
    ).to(device)
